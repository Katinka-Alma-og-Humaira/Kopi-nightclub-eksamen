...
const RecentBlogs = async () => {
  const events = await getUpcommingEvents(); // <-- henter data, ligesom UpcommingEvents.jsx gør
        ...
        {recentEvents.map((event) => (
          <div key={event.slug} className="mb-10">
            ...
            <h3 className="mb-(--space-m) text-(--color-pink)!">
              <span>BY: Admin </span>
              <span>/ </span>
              <span> {event.comments && event.comments > 0 ? `${event.comments} Comments` : "No Comments "}</span>
              <span>/ </span>
