...
export default function NightclubGallery({ gallery }) {
  ...
  const GalleryItem = ({ item, index, heightClass }) => (
    ...
    >
      <motion.div className="absolute inset-0" variants={{ rest: { opacity: 0 }, hover: { opacity: 1 } }} transition={{ duration: 0.3, ease: "easeOut" }} style={{ backgroundColor: "rgba(0, 0, 0, 0.45)" }} />
      ...
      <motion.div className="absolute top-0 left-0 w-0 h-0" style={{ borderTop: "50px solid var(--color-pink)", borderRight: "50px solid transparent", transformOrigin: "top left" }} variants={{ rest: { scale: 0 }, hover: { scale: 1 } }} transition={{ duration: 0.3, ease: "easeOut" }} />
      <motion.div className="absolute bottom-0 right-0 w-0 h-0" style={{ borderBottom: "50px solid var(--color-pink)", borderLeft: "50px solid transparent", transformOrigin: "bottom right" }} variants={{ rest: { scale: 0 }, hover: { scale: 1 } }} transition={{ duration: 0.3, ease: "easeOut" }} />
    </motion.div>
  );

  return (
    <div className="flex flex-col items-center w-full lg:max-w-[1500px] max-h-[1000px] lg:mx-auto">
      ...
      <Dialog.Root open={selectedIndex !== null} onOpenChange={(open) => !open && setSelectedIndex(null)}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed z-50 top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] md:w-[700px] bg-black outline-none overflow-visible">
            {selectedIndex !== null && (
              <>
                <button onClick={prev} className="absolute -left-8 md:-left-55 top-1/2 -translate-y-1/2 text-white p-3 border border-white/40 bg-transparent">
                  ◀
                </button>
                <button onClick={next} className="absolute -right-8 md:-right-55 top-1/2 -translate-y-1/2 text-white p-3 border border-white/40 bg-transparent">
                  ▶
                </button>
