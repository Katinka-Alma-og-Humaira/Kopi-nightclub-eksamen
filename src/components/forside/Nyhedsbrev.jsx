...
// AI har hjulpet lidt med validering
...
const Newsletter = () => {
  ...
  const handleSubscribe = async () => {
    ...
    if (!result.success) {
      setStatus({ type: "invalid", message: result.error.issues[0].message });
      return;
    }
  ...
  return (
    <section className="flex flex-col justify-center items-center gap-3 py-10 m-5 ">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-end gap-10 mt-10">
        <input className="border-b-2 border-white outline-none p-2 w-100 placeholder-white text-white focus:outline-none focus:ring-2 focus:ring-(--color-pink)" type="email" placeholder="Enter your Email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleSubscribe()} disabled={loading} />
