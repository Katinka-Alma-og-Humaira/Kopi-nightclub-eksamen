...
const BookATable = ({ events, selectedTable, preselectedEventId, onReservationSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setForm({ name: "", email: "", phone: "", table: "", guests: "", eventId: "" });
        setErrors({});
      } else {
        setStatus("error");
      }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <select name="eventId" value={form.eventId} onChange={handleChange} disabled={!!preselectedEventId} className={`${inputClass} text-[var(--color-neutrals-200)]`}>
      ...
      <textarea name="comment" placeholder="Your Comment" className={`${inputClass} resize-y min-h-[160px]`} />
