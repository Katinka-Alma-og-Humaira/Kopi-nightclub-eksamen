...
const BookATable = ({ events, selectedTable, selectedEventFromBookNow, onReservationSuccess, onEventChange }) => {
  ...
  useEffect(() => {
    if (selectedEventFromBookNow) {
      setForm((prev) => ({ ...prev, eventId: selectedEventFromBookNow }));
      onEventChange?.(selectedEventFromBookNow);
    }
  }, [selectedEventFromBookNow]);
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-col-1 md:grid-cols-2 gap-4">
        <div>
          <select name="eventId" value={form.eventId} onChange={handleChange} className={`${inputClass} text-[var(--color-neutrals-200)]`}>
      ...
      <textarea name="comment" placeholder="Your Comment" value={form.comment} onChange={handleChange} className={`${inputClass} w-full resize-y min-h-[160px]`} />
