  ...
  return (
    <div>
      {currentEvents.map((event, index) => (
        <div key={event.slug} className={`my-20 grid grid-cols-1 min-[570px]:grid-cols-2 gap-5 items-start ${index % 2 !== 0 ? "min-[570px]:[&>*:first-child]:order-2" : ""}`}>
          <div>
            <Link href={`/detail-event/${event.slug}`}>
              <Image className="my-8 min-[570px]:my-0" src={`${process.env.NEXT_PUBLIC_API_URL}${event.asset.url}`} alt={event.asset.alt} width={event.asset.width} height={event.asset.height} />
            </Link>
          </div>
          <div>
